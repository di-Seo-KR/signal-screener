  ...
  return (
    <div style={{ maxWidth: 920, margin: "0 auto", padding: "16px" }}>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12, flexWrap: "wrap" }}>
          ...
          <button onClick={() => setShowSettings(true)} style={{
            ...
            fontWeight: 600, fontSize: FONT.sm, cursor: "pointer",
          }}>⚙️ 알림 설정</button>
        </div>
      </div>
      ...
      <div style={{
        ...
        <FilterChip C={C} active={filter === "all"} onClick={() => setFilter("all")}>전체</FilterChip>
        <FilterChip C={C} active={filter === "unread"} onClick={() => setFilter("unread")}>
          안 읽음 {unreadCount > 0 && `(${unreadCount})`}
        </FilterChip>
        <FilterChip C={C} active={filter === "priority:high"} onClick={() => setFilter("priority:high")}>🚨 긴급</FilterChip>
        {Object.entries(CATEGORY_META).map(([k, m]) => (
          <FilterChip key={k} C={C} active={filter === `category:${k}`} onClick={() => setFilter(`category:${k}`)}>
            {m.icon} {m.label}
          </FilterChip>
        ))}
        <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
          {unreadCount > 0 && (
            <button onClick={markAllRead} style={btnGhost(C)}>모두 읽음</button>
          )}
          {items.length > 0 && (
            <button onClick={clearAll} style={btnGhost(C)}>전체 삭제</button>
          )}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {filtered.map(n => (
            <NotificationCard key={n.id} n={n} C={C} onClick={() => handleClick(n)} />
          ))}
        </div>
      )}

      {showSettings && (
        <SettingsModal C={C} uid={uid} onClose={() => setShowSettings(false)} />
      )}
    </div>
  );
}
...
function FilterChip({ C, active, onClick, children }) {
  return (
...
function NotificationCard({ n, C, onClick }) {
...
// 알림 설정 모달
// ────────────────────────────────────────────────
function SettingsModal({ C, uid, onClose }) {
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        {!prefs ? (
          ...
          <>
            ...
            >
              <NumberInput C={C} value={prefs.price.threshold_pct} min={0.5} max={50} step={0.5} unit="%"
            ...
            >
              <NumberInput C={C} value={prefs.signal.min_score} min={50} max={100} step={5}
            ...
            >
              <NumberInput C={C} value={prefs.portfolio.threshold_pct} min={-50} max={0} step={1} unit="%"
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 20, justifyContent: "flex-end" }}>
              <button onClick={onClose} style={btnGhost(C)}>취소</button>
...
function NumberInput({ C, value, onChange, min, max, step = 1, unit }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
      <input type="number" value={value} min={min} max={max} step={step}
        ...
        style={{
          ...
          color: C.text1, fontSize: FONT.sm, width: 90,
        }} />
...
function selectStyle(C) {
  return {
    padding: "6px 10px", borderRadius: RADIUS.sm,
    background: C.card, border: `1px solid ${C.border2}`,
    color: C.text1, fontSize: FONT.sm,
  };
}
