...
// ════════════════════════════════════════════════════════════════════
// ── 퀵 투자진단 (카드용 — API 호출 없이 기존 데이터로 즉시 계산) ──
function quickDiagnosis(asset) {
  ...
  if (asset.ma200Dist != null) {
    if (asset.ma200Dist > 10) { trendScore += 20; signals.push({ type: "bullish", name: "200일선 위 +10% 이상" }); }
    else if (asset.ma200Dist > 0) trendScore += 10;
    else if (asset.ma200Dist < -10) { trendScore -= 15; signals.push({ type: "bearish", name: "200일선 아래 -10% 이상" }); }
    else trendScore -= 5;
  }
  if (asset.ma50 && asset.ma200 && asset.ma50 > asset.ma200) { trendScore += 10; signals.push({ type: "bullish", name: "골든크로스 구간" }); }
  if (asset.ma50 && asset.ma200 && asset.ma50 < asset.ma200) { trendScore -= 5; signals.push({ type: "bearish", name: "데스크로스 구간" }); }
  if (asset.weekChange > 3) trendScore += 5;
  if (asset.weekChange < -3) trendScore -= 5;
  trendScore = Math.max(0, Math.min(100, trendScore));

  // 모멘텀: RSI + 스토캐스틱
  if (asset.rsi != null) {
    if (asset.rsi >= 70) { momScore -= 15; signals.push({ type: "bearish", name: `RSI 과매수 (${asset.rsi})` }); }
    else if (asset.rsi <= 30) { momScore += 15; signals.push({ type: "bullish", name: `RSI 과매도 (${asset.rsi})` }); }
    else if (asset.rsi > 50) momScore += 8;
    else momScore -= 5;
  }
  if (asset.stoch?.k != null) {
    if (asset.stoch.k < 20) momScore += 8;
    if (asset.stoch.k > 80) momScore -= 8;
  }
  if (asset.wr != null) {
    if (asset.wr < -80) momScore += 5;
    if (asset.wr > -20) momScore -= 5;
  }
  momScore = Math.max(0, Math.min(100, momScore));

  // 수급
  if (asset.volRatio >= 2) { supScore += 20; signals.push({ type: "bullish", name: `거래량 급증 (${asset.volRatio}x)` }); }
  else if (asset.volRatio >= 1.5) supScore += 10;
  else if (asset.volRatio <= 0.3) { supScore -= 10; signals.push({ type: "neutral", name: "거래량 고갈" }); }
  if (asset.weekChange > 0 && asset.volRatio > 1.2) supScore += 10;
  ...
  if (asset.low52w && asset.high52w) {
    ...
    if (pos52 <= 15) { posScore += 15; signals.push({ type: "bullish", name: "52주 저점 근접" }); }
    if (pos52 >= 90) { posScore -= 5; signals.push({ type: "neutral", name: "52주 고점 근접" }); }
    const fromHigh = ((asset.price - asset.high52w) / asset.high52w) * 100;
    if (fromHigh < -30) { posScore += 10; signals.push({ type: "bullish", name: `고점 대비 ${fromHigh.toFixed(0)}%` }); }
  }
  ...
  let verdict;
  if (totalScore >= 75) verdict = "적극 매수";
  else if (totalScore >= 60) verdict = "매수 우위";
  else if (totalScore >= 45) verdict = "중립";
  else if (totalScore >= 30) verdict = "매도 우위";
  else verdict = "적극 매도";

  return { score: totalScore, verdict, signals, categories: [
                    ...
                    <div style={{ display: "flex", alignItems: "baseline", gap: "10px", flexWrap: "wrap" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                        ...
                        <span style={{ fontSize: "11px", color: C.text3 }}>
                          ({pnlAmt >= 0 ? "+" : ""}${Math.abs(pnlAmt).toLocaleString(undefined, { maximumFractionDigits: 0 })})
                        </span>
                      </div>
                    </div>
                  ) : (
