...
export default function Onboarding({ onClose, onNavigate, isOwner = false }) {
  ...
  // ★ 2026-07 정보 서비스 피벗: 비owner(일반 유저)는 step 3(위험 성향·추천 봇)과
  //   step 4(Binance 연결)를 건너뜀 — 매매 봇 추천·자동매매 소구를 공개 온보딩에서 제거.
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", gap: 10 }}>
            ...
            <button onClick={() => {
              ...
              // ★ 2026-07 정보 서비스 피벗: 자동매매 대신 코인 분석(스크리너)으로 이동
    ...
    if (step === 5) {
      ...
      return (
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* ★ 2026-07 정보 서비스 피벗: 추천 봇(매매 전략) 카드는 내부 운영(owner) 전용 —
              비owner 에게는 매매 소구 없이 관심 자산 기반 중립 안내만 표시 */}
          {isOwner ? (
            <div style={{
