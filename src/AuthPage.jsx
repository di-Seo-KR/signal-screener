...
export default function AuthPage({ theme = "dark", embedded = false, onClose }) {
  ...
  const S = {
    ...
    oauthBtn: (bgColor) => ({
      ...
      gap: "8px", padding: "12px", borderRadius: "10px", fontSize: "13px",
      fontWeight: 600, border: `1px solid ${C.border}`, cursor: "pointer",
      background: bgColor, color: C.text1, transition: "all 0.2s",
    }),
    error: {
          ...
          {mode !== "reset" && (
            <>
              ...
              <div style={S.oauthRow}>
                <button onClick={() => handleOAuth("google")} style={S.oauthBtn(C.card2)} disabled={loading}>
                ...
                <button onClick={() => handleOAuth("github")} style={S.oauthBtn(C.card2)} disabled={loading}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill={C.text1}>
                    <path d="M12 0C5.37 0 0 5.37 0 12c0 5.31 3.43 9.8 8.21 11.39.6.11.79-.26.79-.58 0-.29-.01-1.24-.01-2.25-3.34.73-4.04-1.41-4.04-1.41-.55-1.39-1.34-1.76-1.34-1.76-1.09-.75.08-.73.08-.73 1.21.08 1.85 1.24 1.85 1.24 1.07 1.84 2.81 1.31 3.5 1 .11-.78.42-1.31.76-1.61-2.67-.3-5.47-1.33-5.47-5.93 0-1.31.47-2.38 1.24-3.22-.12-.3-.54-1.52.12-3.18 0 0 1.01-.32 3.3 1.23a11.5 11.5 0 0 1 6.02 0c2.28-1.55 3.29-1.23 3.29-1.23.66 1.66.24 2.88.12 3.18.77.84 1.24 1.91 1.24 3.22 0 4.61-2.81 5.63-5.48 5.92.43.37.81 1.1.81 2.22 0 1.6-.01 2.89-.01 3.29 0 .32.19.7.8.58C20.57 21.8 24 17.31 24 12c0-6.63-5.37-12-12-12z"/>
                  </svg>
                  GitHub
                </button>
              </div>
