...
//              ※ **그 경로에서 실제로 보이는 기능만** 적습니다. 다른 라우트의 기능을
//                섞으면 검색 유입 사용자가 약속과 다른 화면에 도착합니다(허위 표기).
//   faq      : Q&A 3~4개 (Google FAQ rich snippet 대상)
const ROUTES = [
  // ══════════════════════════════════════════════════════════════════
  // ★ 2026-08-13 SEO 감사 #15 대응 — IA v3 주요 경로가 홈과 똑같은 제네릭
  //   title 로 200 을 반환하던 문제(소프트 404·중복 title) 수정.
  //   원인: /crypto·/stocks·/indicators 는 App.jsx 가 실제로 라우팅하는데
  //   이 라우트 테이블에 없어 vercel.json 의 catch-all 이 index.html(홈 메타)로
  //   흘려보내고 있었습니다. 아래 3개를 정본(canonical)으로 추가하고, 구 경로
  //   (/screener·/news·/econ-calendar)는 canonicalPath 로 정본을 가리키게 해
  //   중복 색인을 정리했습니다.
  //   ※ /stock·/report·/riskmap 은 앱에 존재하지 않는 경로라 여기서 만들지
  //     않았습니다. vercel.json 에서 실재 경로로 301 리다이렉트합니다.
  // ══════════════════════════════════════════════════════════════════
  {
    path: "stocks", // IA v3 주식 탭 정본 (내부 tab id 는 screener)
    title: "주식 시그널·스크리너 — 미국·한국 종목 멀티 타임프레임 | Zepta",
    desc: "미국 S&P 500 전 종목과 한국 코스피·코스닥 시총 상위 종목을 주봉·일봉·4시간·1시간 종합 스코어로 정리합니다. 방향과 강도를 한 화면에서 확인하세요.",
    h1: "Zepta 주식 — 미국·한국 종목 시그널을 한 화면에",
    intro: "미국 S&P 500 전 종목과 한국 코스피·코스닥 시가총액 상위 종목을 기술적 지표로 스캔합니다. 주봉·일봉·4시간·1시간 네 개 타임프레임의 방향이 서로 일치하는지까지 함께 보여줍니다.",
    features: [
      "멀티 타임프레임 종합 — 주봉·일봉·4시간·1시간의 방향을 가중 합산한 종합 스코어",
      "타임프레임 분해 — 어느 시간대가 위·아래를 가리키는지 그대로 공개",
      "지지·저항 — 최근 가격 구조에서 뽑은 지지선·저항선과 현재가 대비 거리",
      "미국·한국 이원화 — 장중인 시장을 먼저, 마감한 시장은 마지막 데이터 기준으로 표기",
      "신규 상장 포함 — 2025~2026년 상장 종목도 데이터가 쌓이는 대로 편입",
    ],
    faq: [
      { q: "어떤 종목을 분석하나요?", a: "미국은 S&P 500 전 종목에 시가총액·거래대금 상위 종목과 최근 상장 종목을 더했고, 한국은 코스피·코스닥 시가총액 상위 종목을 봅니다. 모든 티커는 실제 시세 응답이 확인된 것만 등재합니다." },
      { q: "스코어는 무엇으로 계산되나요?", a: "추세(이동평균 배열·교차), 모멘텀(RSI·MACD·스토캐스틱), 거래량(OBV·거래량 급증), 변동성(ATR·볼린저밴드) 같은 기술적 지표만 사용합니다. 재무제표나 실적 전망은 반영하지 않습니다." },
      { q: "장이 마감된 뒤에는 어떤 값이 보이나요?", a: "마지막 장중 데이터 기준 값이 그대로 유지됩니다. 화면에는 데이터 기준 시각이 함께 표기되므로 언제 시점의 값인지 확인할 수 있습니다." },
    ],
  },
  {
    path: "crypto", // IA v3 코인 탭 정본 (내부 tab id 는 news)
    title: "코인 시그널·파생 지표 — 김치 프리미엄·펀딩비·미결제약정 | Zepta",
    desc: "주요 암호화폐의 멀티 타임프레임 시그널과 김치 프리미엄·펀딩비·미결제약정·롱숏 비율 등 파생 지표를 한 화면에서 확인하세요.",
    h1: "Zepta 코인 — 시그널과 파생 지표를 한 화면에",
    intro: "유동성 상위 암호화폐를 주봉·일봉·4시간·1시간 기준으로 분석하고, 김치 프리미엄과 펀딩비·미결제약정 같은 파생 시장 지표를 함께 보여줍니다.",
    features: [
      "코인별 종합 스코어 — 네 개 타임프레임의 방향을 가중 합산",
      "김치 프리미엄 — 국내·해외 거래소 가격 차이를 실시간으로",
      "펀딩비·미결제약정 — 선물 시장의 쏠림과 과열 정도",
      "롱숏 비율 — 포지션이 한쪽으로 몰렸는지 확인",
      "종목별 상세 — 코인 개별 페이지에서 지표 근거를 그대로 공개",
    ],
    faq: [
      { q: "어떤 코인을 다루나요?", a: "거래대금 기준 유동성 상위 코인을 자동으로 선별해 분석합니다. 목록은 시장 상황에 따라 주기적으로 갱신됩니다." },
      { q: "김치 프리미엄은 어떻게 계산하나요?", a: "국내 거래소 원화 가격과 해외 거래소 달러 가격을 실시간 환율로 환산해 비교한 차이입니다." },
      { q: "이 지표만 보고 매매해도 되나요?", a: "아니요. Zepta는 투자 정보·데이터 서비스이며 매매를 권유하지 않습니다. 모든 수치는 참고용이고 최종 판단과 책임은 본인에게 있습니다." },
    ],
  },
  {
    // IA v3 지표 탭 — 실제 화면은 경제 캘린더 | 마켓 뉴스 두 서브탭이 전부입니다
    // (App.jsx: indicatorsView === "calendar" | "news").
    // ★ 2026-08-13 리뷰 수리: features 에 이 페이지에 없는 기능(시장 심리·이상 감지·
    //   리스크 맵)이 3개 섞여 있었습니다. 셋 다 /sentiment·/anomaly·/risk-map 별도
    //   라우트이고 각자 전용 prerender 페이지도 이미 있습니다. 검색 유입 사용자가
    //   약속된 기능을 보고 들어왔다가 캘린더·뉴스만 보게 되는 허위 표기라, 실제
    //   화면이 제공하는 것으로 교체하고 별도 페이지 위치를 명시했습니다.
    //   (뉴스 정렬 FAQ 의 "시장 영향도 기준 정렬"도 실제로는 최신·긍정·부정 논조
    //    정렬이라 함께 정정했습니다.)
    path: "indicators",
    title: "지표 — 경제 캘린더·마켓 뉴스 | Zepta",
    desc: "FOMC·CPI·고용지표 등 주요 경제 지표 발표 일정과 마켓 뉴스를 한 화면에서 확인하세요. 한국 표준시(KST) 기준으로 정리합니다.",
    h1: "Zepta 지표 — 경제 캘린더와 마켓 뉴스",
    intro: "지표 탭은 경제 캘린더와 마켓 뉴스 두 화면으로 구성됩니다. 시장을 움직이는 지표 발표 일정과 헤드라인을 자산군 구분 없이 한 곳에서 확인하세요.",
    features: [
      "경제 캘린더 — FOMC·CPI·고용지표 등 주요 발표 일정을 한국 표준시(KST)로",
      "일정 필터 — 경제지표·FOMC·CPI·고용·GDP 로 좁혀 보기, 날짜별 선택",
      "발표 결과 아카이브 — 지난 발표의 실제 수치를 모아 둔 페이지로 연결",
      "마켓 뉴스 — 미국·한국·코인 카테고리별 헤드라인",
      "뉴스 정렬 — 최신순과 논조(긍정·부정) 기준 정렬, 논조 구성 요약",
    ],
    faq: [
      { q: "경제 캘린더 시각은 어느 기준인가요?", a: "한국 표준시(KST) 기준으로 표시합니다." },
      { q: "구 주소 /econ-calendar 로 들어가도 되나요?", a: "네. 기존 북마크는 그대로 동작하며 지표 탭의 캘린더 화면으로 연결됩니다." },
      { q: "뉴스는 어떻게 정렬되나요?", a: "최신순이 기본이며, 제목 논조 기준으로 긍정·부정 순 정렬도 선택할 수 있습니다." },
      { q: "시장 심리나 리스크 맵은 어디에 있나요?", a: "지표 탭이 아닌 별도 페이지입니다. 시장 심리는 /sentiment, 이상 감지는 /anomaly, 리스크 맵은 /risk-map 에서 제공합니다." },
    ],
  },
  {
    path: "screener",
    canonicalPath: "stocks", // IA v3 에서 /stocks 가 정본 — 구 주소는 중복 색인 방지용 별칭
    title: "주식 시그널·스크리너 — 미국·한국 종목 멀티 타임프레임 | Zepta",
    desc: "미국 S&P 500 전 종목과 한국 코스피·코스닥 시총 상위 종목을 주봉·일봉·4시간·1시간 종합 스코어로 정리합니다.",
    h1: "Zepta 주식 — 미국·한국 종목 시그널을 한 화면에",
    intro: "미국 S&P 500 전 종목과 한국 코스피·코스닥 시가총액 상위 종목을 기술적 지표로 스캔합니다. 이 주소(/screener)는 /stocks 의 구 주소로, 같은 화면으로 연결됩니다.",
    features: [
      "멀티 타임프레임 종합 — 주봉·일봉·4시간·1시간의 방향을 가중 합산한 종합 스코어",
      "타임프레임 분해 — 어느 시간대가 위·아래를 가리키는지 그대로 공개",
      "조건 스크리닝 — 가격·거래량·변동성·모멘텀 조건으로 종목 탐색",
      "지지·저항 — 최근 가격 구조에서 뽑은 지지선·저항선과 현재가 대비 거리",
      "미국·한국 통합 — 두 시장을 같은 기준으로 비교",
    ],
    faq: [
      { q: "/screener 와 /stocks 는 무엇이 다른가요?", a: "같은 화면입니다. IA 개편으로 주식 탭 주소가 /stocks 로 바뀌었고 /screener 는 기존 북마크 호환을 위해 유지됩니다." },
      { q: "어떤 종목을 분석하나요?", a: "미국은 S&P 500 전 종목에 시가총액·거래대금 상위 종목과 최근 상장 종목을 더했고, 한국은 코스피·코스닥 시가총액 상위 종목을 봅니다." },
      { q: "스코어는 무엇으로 계산되나요?", a: "추세·모멘텀·거래량·변동성 계열의 기술적 지표만 사용합니다. 재무제표나 실적 전망은 반영하지 않습니다." },
    ],
  },
  {
    path: "auto-trading",
  ...
  },
  {
    ...
    intro: "RSI, MACD, 볼린저밴드, Hurst 지수, Efficiency Ratio, OBV 등 검증된 9개 알파 시그널을 family 별로 정리했습니다. 각 시그널의 작동 원리와 백테스트 성과를 확인하세요.",
    features: [
    ...
    faq: [
      { q: "9개 시그널 중 어떤 것이 가장 좋나요?", a: "시장 상황(추세/횡보)에 따라 최적 시그널이 달라집니다. Alpha Lab leaderboard 에서 현재 시장 레짐에 맞는 상위 시그널을 확인하세요." },
  ...
  {
    path: "econ-calendar",
    canonicalPath: "indicators", // IA v3: 경제 캘린더는 지표 탭 서브탭이 흡수 — 정본은 /indicators
  ...
  {
    path: "news",
    // ★ 2026-08-13 정정: IA v3 에서 /news 는 **코인 탭**으로 라우팅됩니다
    //   (App.jsx PATH_TO_TAB: news → 코인). 시장 뉴스는 지표 탭(/indicators?view=news)
    //   이 흡수했습니다. 기존 메타는 "시장 뉴스 페이지"라고 안내해 실제 도착 화면과
    //   달랐으므로(허위 표기) 코인 탭 내용으로 바로잡고 정본을 /crypto 로 넘깁니다.
    canonicalPath: "crypto",
    title: "코인 시그널·파생 지표 — 김치 프리미엄·펀딩비·미결제약정 | Zepta",
    desc: "주요 암호화폐의 멀티 타임프레임 시그널과 김치 프리미엄·펀딩비·미결제약정·롱숏 비율 등 파생 지표를 한 화면에서 확인하세요.",
    h1: "Zepta 코인 — 시그널과 파생 지표를 한 화면에",
    intro: "이 주소(/news)는 코인 탭의 구 주소입니다. 유동성 상위 암호화폐를 주봉·일봉·4시간·1시간 기준으로 분석하고 파생 시장 지표를 함께 보여줍니다. 시장 뉴스는 지표 탭으로 옮겨졌습니다.",
    features: [
      "코인별 종합 스코어 — 네 개 타임프레임의 방향을 가중 합산",
      "김치 프리미엄 — 국내·해외 거래소 가격 차이를 실시간으로",
      "펀딩비·미결제약정 — 선물 시장의 쏠림과 과열 정도",
      "롱숏 비율 — 포지션이 한쪽으로 몰렸는지 확인",
      "시장 뉴스 — 지표 탭(/indicators)의 뉴스 화면으로 이동",
    ],
    faq: [
      { q: "/news 로 들어왔는데 코인 화면이 나옵니다.", a: "IA 개편으로 /news 는 코인 탭 주소가 되었습니다. 시장 뉴스는 지표 탭(/indicators)의 뉴스 화면에서 볼 수 있습니다." },
      { q: "어떤 코인을 다루나요?", a: "거래대금 기준 유동성 상위 코인을 자동으로 선별해 분석합니다. 목록은 시장 상황에 따라 주기적으로 갱신됩니다." },
      { q: "김치 프리미엄은 어떻게 계산하나요?", a: "국내 거래소 원화 가격과 해외 거래소 달러 가격을 실시간 환율로 환산해 비교한 차이입니다." },
    ],
  },
  ...
  },
  {
    ...
    features: [
      ...
      "시그널 알림 — 9개 알파 시그널 중 선택해 발생 시 알림",
  ...
  return `
    <section class="seo-prerender" style="display:block;padding:24px;max-width:800px;margin:0 auto;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;color:#F4F5F7;background:#0A0B0F;line-height:1.7;">
      ...
      <nav style="margin-top:32px;padding-top:16px;border-top:1px solid #23262F;font-size:14px;color:#A1A6B2;">
        ...
        <a href="/stocks" style="color:#6E92FF;margin-right:12px;">주식</a>
        <a href="/crypto" style="color:#6E92FF;margin-right:12px;">코인</a>
        <a href="/indicators" style="color:#6E92FF;margin-right:12px;">지표</a>
...
const HOME = {
  ...
  guides: [
    ...
    // ★ 2026-08-13: "33가지"는 근거 없는 숫자라 제거 (전략 수 표기 기준 전수 정정 건)
    { href: "/blog/quant-strategies", title: "퀀트 전략 정리" },
