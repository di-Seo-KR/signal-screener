...
function buildCoinPage(coin, idx) {
  ...
  const faq = [
    { q: `${ko}(${sym}) 선물을 Zepta에서 자동매매할 수 있나요?`,
      a: `네. ${ko}는 Zepta가 다루는 ${COINS.length}개 메이저 코인 중 하나로, 바이낸스 USDM 선물 ${fut} 심볼로 거래됩니다. 봇이 주봉·일봉·4시간·1시간을 종합한 스코어로 롱·숏을 자동 판단하고, 레버리지 5배·격리마진으로 진입합니다.` },
...
${cards}
    ...
    <p>${listText} — 메이저 ${COINS.length}종은 코인별 정체·합의 방식·매매 시 주의점까지 담은 상세 페이지를 제공합니다. 그 외 유동성 상위 신규 코인은 위 대시보드에서 실시간 스코어로 확인할 수 있습니다.</p>
