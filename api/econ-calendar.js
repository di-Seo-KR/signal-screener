// Vercel Serverless — 경제 캘린더 API (멀티소스)
// GET /api/econ-calendar
// 1차: Finnhub (무료 60req/min)
// 2차: FinancialModelingPrep
// 3차: 큐레이션 폴백 (실제 발표 수치 반영)
...
export default async function handler(req, res) {
  ...
  // ── 1차: Finnhub 경제 캘린더 ──
  try {
    const finnhubUrl = `https://finnhub.io/api/v1/calendar/economic?from=${fmtDate(from)}&to=${fmtDate(to)}&token=demo`;
    ...
    if (resp.ok) {
      ...
      if (Array.isArray(data) && data.length > 5) {
        ...
        if (filtered.length > 0) {
          return res.status(200).json({ events: filtered, source: "finnhub" });
        }
      }
    }
  ...
  try {
    ...
    const resp = await fetch(fmpUrl, { signal: AbortSignal.timeout(6000) });
    ...
    if (resp.ok) {
      ...
      if (Array.isArray(data) && data.length > 0) {
        ...
        if (filtered.length > 0) {
          return res.status(200).json({ events: filtered, source: "fmp" });
        }
      }
    }
  } catch (_) { /* FMP 실패 → 폴백 */ }
...
function getCuratedEvents2026() {
  ...
  return [
    ...
    { date: "2026-03-11", event: "CPI (YoY)", actual: 2.4, estimate: 2.9, previous: 3.0, impact: "High", unit: "%" },
    { date: "2026-03-11", event: "Core CPI (YoY)", actual: 2.5, estimate: 3.2, previous: 3.3, impact: "High", unit: "%" },
    ...
    { date: "2026-04-03", event: "Unemployment Rate", actual: null, estimate: 4.1, previous: 4.1, impact: "High", unit: "%" },
    { date: "2026-04-10", event: "CPI (YoY)", actual: null, estimate: 2.8, previous: 2.4, impact: "High", unit: "%" },
    { date: "2026-04-10", event: "Core CPI (YoY)", actual: null, estimate: 2.6, previous: 2.5, impact: "High", unit: "%" },
    ...
    { date: "2026-04-30", event: "PCE Price Index (YoY)", actual: null, estimate: 2.4, previous: 2.5, impact: "High", unit: "%" },
