...
// 핵심 시장 뉴스만 필터링하여 반환
//
// ★ 2026-06-12 성능 개편 (대표 지시 — 콜드 5.04s → ~1s):
//   1) Nitter(X) 소스 삭제 — 2024년 X 게스트 폐지 후 공개 인스턴스 전멸(403/DNS 실패).
//      매 콜드마다 최대 9회 직렬 호출로 ~3초를 태우면서 뉴스 0건 기여하던 죽은 코드.
//   2) 소스 간 의존성이 없으므로 Yahoo / Google(3쿼리) / CoinGecko 전부 병렬
//      (Promise.allSettled). 병합 순서는 기존 dedup 우선순위(Yahoo 우선) 그대로 보존.
//   3) 모든 fetch 에 AbortSignal.timeout(2500) — worst-case 를 2.5s 로 캡.
//   4) SWR 600→86400: 캐시 만료 후에도 만료본 즉시 반환 + 백그라운드 재검증
//      (15분 무트래픽·배포 퍼지 후 첫 방문자가 풀 콜드를 맞던 문제 해소).
  ...
  items.slice(0, 15).forEach(item => {
    ...
    if (title && link) {
      ...
      if (isRelevant(title, cleanDesc, "en")) {
        ...
        out.push({ title: title.trim(), url: link.trim(), date: pubDate || "", desc: cleanDesc, source: "Yahoo Finance", tags });
      }
    }
  });
  return out;
...
// Source 2: Google News RSS — 쿼리 1개
async function fetchGoogleQuery(q, lang) {
  ...
  items.slice(0, 8).forEach(item => {
    ...
    if (title && isRelevant(title, "", lang)) {
      ...
      out.push({ title: title.trim(), url: (link || "").trim(), date: pubDate || "", desc: "", source: source || "Google News", tags });
    }
  });
  return out;
}

// Source 3: CoinGecko trending top 3
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 병합 순서 고정: Yahoo → Google(쿼리순) → CoinGecko (기존 dedup 우선순위 보존)
    ...
    // ★ SWR 24h — 만료 후에도 만료본 즉시 반환(0.04s) + 백그라운드 재검증
