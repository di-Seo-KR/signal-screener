...
export default async function handler(req, res) {
  ...
  try {
    ...
    const tgMsg = [
      ...
      ``,
      `${latestSignal.type === "BUY" ? "🟢" : "🔴"} ${latestSignal.type} 시그널 [${latestSignal.confidence}급]`,
      ...
      `📊 계좌: $${equity.toFixed(0)}`,
      btcPos ? `₿ 보유: $${parseFloat(btcPos.market_value || 0).toFixed(0)} (P&L: $${parseFloat(btcPos.unrealized_pl || 0).toFixed(2)})` : "₿ 보유 없음",
      ``,
